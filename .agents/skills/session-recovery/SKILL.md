...
**路径**: `.Codex/session-state.md`（项目根目录下）
...
## 分支与工作空间
- **Branch**: feature/xxx
- **Worktree**: .Codex/worktrees/xxx（如适用）
...
```dot
digraph recovery {
    "检查 .Codex/session-state.md 是否存在" [shape=diamond];
    ...
    "检查 .Codex/session-state.md 是否存在" -> "无活跃工作流，正常响应" [label="不存在"];
    "检查 .Codex/session-state.md 是否存在" -> "读取 session state" [label="存在"];
...
1. **读取 `.Codex/session-state.md`** — 获取工作流模式、plan 路径、当前进度
