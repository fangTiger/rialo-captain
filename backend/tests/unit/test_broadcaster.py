...
def test_event_type_enum_values():
    ...
    assert EventType.POOL_OPENED.value == "pool.opened"
    assert EventType.POOL_POLICY_BOUND.value == "pool.policy_bound"
    assert EventType.POOL_CLAIM_PAID.value == "pool.claim_paid"
    assert EventType.POOL_RULE_UPDATED.value == "pool.rule_updated"
    assert EventType.POOL_CLOSED.value == "pool.closed"
