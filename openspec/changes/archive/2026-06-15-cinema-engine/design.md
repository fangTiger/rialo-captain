...
- `/` 默认进入 cinema 态，30s 一 cycle 自动选主角、推镜头、触发 demo seed/delay，并循环。
...
### 2. CameraDirector 驱动现有 `GlobeMap` viewport
...
`CameraDirector` 根据 protagonist 经纬度与当前 `GlobeMap` 尺寸计算目标 viewport，传入 `cameraTarget`。`GlobeMap` 收到 target 后用 `requestAnimationFrame` 插值 `k/x/y`，且当用户手势发生时取消动画并通知 controller。

这样保留现有投影、tooltip、hover、点击导航逻辑。备选方案是迁移到 Mapbox/deck.gl camera；范围过大，且会影响当前已通过的 Tower 测试。
...
- 当前 phase 为 `establish` 或 `rest` 时，REAL 事件立即成为当前或下一 cycle 主角。
- 当前 phase 为 `story`、`zoom-in`、`zoom-out` 时，REAL 入队，当前故事不被打断。
...
- `policy.created` 且 `source === "real"` → 入 REAL protagonist queue。
...
- `GlobeMap` 当前投影和 viewport 状态在组件内部 → 需要小心抽出 target 应用逻辑，避免测试环境缺少 layout 导致动画不稳定；用 deterministic helper 测 viewport 计算。
...
5. 验证后端 pytest、前端 vitest，并在实现阶段用 Playwright smoke 验证 `/` 自动 zoom 与 manual 倒计时。
