...
系统 SHALL 在 cinema STORY phase 开始时为当前主角触发一次 `TrailDraw`，并把播放窗口对齐到 cycle 第 7-10 秒。

#### Scenario: STORY 开始触发 TrailDraw
- **GIVEN** cinema mode 为 `cinema` 且存在当前 protagonist
- **WHEN** phase 从 `zoom-in` 进入 `story`
- **THEN** 系统 SHALL 创建一个 TrailDraw active item
- **AND** 页面 SHALL 渲染 `data-testid="trail-draw"`

#### Scenario: 同一 cycle 和主角不重复触发
- **GIVEN** 当前 cycle/protagonist 已经触发过 TrailDraw
- **WHEN** React 重新渲染或 phase effect 重新执行
- **THEN** 系统 SHALL NOT 创建第二个相同 TrailDraw

#### Scenario: 非 cinema 或无主角不触发
- **GIVEN** 系统处于 interactive、paused-hidden、degraded 或没有 protagonist
- **WHEN** phase 变化或 timer 到达 STORY 窗口
