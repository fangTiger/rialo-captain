...
describe("key moment timeline", () => {
  ...
  it("releases trigger exactly at the 6s STORY window", () => {
    vi.useFakeTimers();
    ...
    expect(state.active[0].startedAt).toBe(cycleStartedAt + 6_000);
  });

  it("releases settled 2s after ShockWave and landed 2s after ChainBeam", () => {
    vi.useFakeTimers();
    vi.setSystemTime(cycleStartedAt);
    ...
    state = advanceAt(state, 9_999);
    expect(state.active.map((moment) => moment.moment.kind)).toEqual([
      "chainbeam",
    ]);

    state = advanceAt(state, 10_000);
    expect(state.active.map((moment) => moment.moment.kind)).toEqual([
      "chainbeam",
      "flareland",
    ]);
